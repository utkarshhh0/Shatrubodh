...
def get_user_alerts(user_id: str) -> pd.DataFrame:
    ...
    try:
        ...
        import sys
        print(f"Error querying user alerts: {e}", file=sys.stderr)
        return pd.DataFrame()
...
def get_user_behavior_history(user_id: str) -> pd.DataFrame:
    ...
    try:
        ...
        import sys
        print(f"Error querying user behavior history: {e}", file=sys.stderr)
        return pd.DataFrame()
...
def render_threat_hunter():
    ...
    try:
        ...
        st.write(
            f"**Name**: {demographics.get('name', 'Unknown')} | "
            f"**Department**: {demographics.get('department', 'Unknown')} | "
            f"**Role**: {demographics.get('role', 'Unknown')} | "
            f"**Manager**: {demographics.get('manager', 'Unknown')} | "
            f"**Team**: {demographics.get('team', 'Unknown')}"
        )
    except Exception as e:
        st.error("Unable to load component.")
        st.exception(e)
    ...
    try:
        ...
        def format_alert_choice(aid):
            ...
            return f"ID: {aid} | Date: {date} | Sev: {sev} | Status: {status}"
...
def render_investigation_view(user_id: str, demographics: dict, alert_data: dict, alert_id: str):
    ...
    if user_history is not None and len(user_history) < 30:
        st.warning(
    ...
    try:
        alert_evidence = parse_evidence(alert_data.get('evidence_json', '{}'))
        ...
        st.markdown(f"**Reasons**: *{alert_data.get('reasons', 'N/A')}*")
    ...
    with col2:
        ...
        try:
            ...
            user_mean = float(df_user[db_col].mean()) if not df_user.empty and db_col in df_user.columns else 0.0
            cohort_mean = float(df_cohort[db_col].mean()) if not df_cohort.empty and db_col in df_cohort.columns else 0.0
