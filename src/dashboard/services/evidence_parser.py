...
def parse_evidence(evidence_json_str: str) -> dict:
    ...
    """
    if not evidence_json_str or not isinstance(evidence_json_str, str):
        return {}
    try:
...
def compute_comparisons(
    ...
    try:
        ...
        for metric in METRICS_COLUMNS:
            x_alert = float(alert_evidence.get(metric, 0)) if alert_evidence else 0.0
            
            # --- A. User Baseline ---
            if user_history_rows and len(user_history_rows) >= 10 and user_means is not None:
            ...
            # --- B. Cohort Baseline ---
            if cohort_history_rows and cohort_means is not None:
