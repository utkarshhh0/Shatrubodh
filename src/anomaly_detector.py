...
class AnomalyDetector:
    ...
    def predict(self, df):
        if self.model is None or self.preprocessor is None:
            try:
                self.model, self.preprocessor = joblib.load(self.model_path)
        ...
        return (pd.Series(risk_scores.flatten(), name='risk_score',
                          index=df.index),
                pd.Series(is_anomaly, name='is_anomaly', index=df.index))
