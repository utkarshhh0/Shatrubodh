...
def generate_mock_data(num_rows=2000):
    ...
    processes = ["powershell.exe", "cmd.exe", "svchost.exe", "python.exe",
                 "ResearchClient.exe", "Game.exe"]

    actions = ["login", "logout", "file_access", "file_upload",
               "file_download", "privilege_change", "process_execution",
               "usb_plugin"]
    ...
    for _ in range(num_rows):
        ...
        action = np.random.choice(actions, p=[0.2, 0.2, 0.2, 0.1, 0.1, 0.05,
                                              0.1, 0.05])
        ...
        record = {
            ...
            'remote_ip': np.random.choice(ips),
        ...
        if action in ["file_access", "file_upload", "file_download"]:
            ...
            record['usb_device'] = f"Kingston_DataTraveler_" \
                                   f"{fake.hexify(text='^^^^')}"

        # Inject anomalies
        if np.random.rand() < 0.05:
            anomaly_type = np.random.choice([
                'off_hours', 'large_download', 'suspicious_process',
                'privilege_abuse'])
            if anomaly_type == 'off_hours':
                ...
                record['bytes_transferred'] = np.random.randint(
                    1024 * 1024 * 50, 1024 * 1024 * 200)
